...
- Use the **global Netlify CLI** from the repo root (`pnpm netlify:link`, `pnpm netlify:sites:create`, `pnpm netlify:deploy`, … — see `wiki/operations.md`). Set secrets with **`netlify env:set`** (or the dashboard); `.netlify/` is gitignored.
