...
- **Current focus:** D4 production smoke (auth + mutation); EAS D5–D7 when ready
...
| D3  | P2       | DONE   | external+code    | D2,B5      | Add Supabase env vars in Netlify.                                                                                               | `pnpm netlify:env:check` exit 0; `pnpm netlify:env:push` (`tooling/netlify-env-push.cjs`, no broken `--scope builds`); prod `**pnpm netlify:deploy:prod`** (`--filter @widados/web`) succeeded deploy `**69e960fccaca132084c45392`** → `https://widados-contacts.netlify.app`.    |
...
- 2026-04-25: **D3** done: `.env.cloud` + Netlify `VITE_SUPABASE_`*; `pnpm netlify:deploy:prod` deploy `**69e960fccaca132084c45392`**; prod JS bundle references **supabase**. **D4** next: manual sign-in + one contact on production URL (optional: `pnpm smoke:netlify-home`).
