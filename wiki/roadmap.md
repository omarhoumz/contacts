...
| D3  | P2       | IN_PROGRESS | external+code    | D2,B5      | Add Supabase env vars in Netlify.                                                                                               | Automation: `pnpm netlify:env:push` (`tooling/netlify-env-push.cjs`, reads `.env.cloud`); `pnpm netlify:env:list`. **DONE when:** push run + list shows `VITE_SUPABASE_*` + prod deploy succeeds.                                                                                    |
