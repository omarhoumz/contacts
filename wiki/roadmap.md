...
Use this template to schedule each next cycle:

```md
[Audit] Full stack tech audit - YYYY-MM
...
| R15 | P1       | DONE         | code+design | R5         | Rewrite to use Tailwind + shadcn patterns where applicable (web-first).                                                                                                                                                                                                                                                                                                                    | Completed monorepo migration slices: web moved to Tailwind utility classes + shadcn-style primitives; runtime `style={{...}}` removed from app UI; `apps/web/src/ui-styles.ts` deleted and theme now class-based (`dark` on root + CSS tokens in `index.css`). Mobile `apps/mobile` screens moved to NativeWind className styling with Babel/Metro/Tailwind setup. Docs updated (`wiki/coding-style.md`, `wiki/operations.md`; migration runbook retired post-R15). Verification PASS: `pnpm --filter @widados/web lint`, `pnpm --filter @widados/web typecheck`, `pnpm --filter @widados/web test`, `pnpm --filter @widados/mobile lint`, `pnpm --filter @widados/mobile typecheck`. |
...
| R9  | P2       | BLOCKED      | code+design | R12        | Dark mode: system preference (`prefers-color-scheme`) on load + manual toggle in sidebar footer, persisted to `localStorage`. Web-first.                                                                                                                                                                                                                                                   | Web slice complete and desktop dark-mode QA PASS. Blocker: browser automation cannot reliably enforce 375×812 viewport (renders ~634×801), preventing trustworthy mobile bottom-nav visual QA. Unblock by validating on real device/manual mobile browser pass or once tooling viewport bug is fixed.                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Styling:** Tailwind utility classes + shadcn-style primitives on web; NativeWind className styling on mobile; `ui-styles.ts` removed. See `wiki/coding-style.md`.
...
- 2026-04-24: `R15` completed in full slices: web runtime styling migrated from inline style maps to Tailwind/shadcn-style primitives, `ui-styles.ts` removed, mobile screens migrated to NativeWind className styling; steady-state docs in `wiki/coding-style.md` + `wiki/operations.md` (migration runbook retired post-R15). Verification PASS: `pnpm --filter @widados/web lint`, `pnpm --filter @widados/web typecheck`, `pnpm --filter @widados/web test`, `pnpm --filter @widados/mobile lint`, `pnpm --filter @widados/mobile typecheck`.
