...
| R9  | P2       | IN_PROGRESS  | code+design | R12        | Dark mode: system preference (`prefers-color-scheme`) on load + manual toggle in sidebar footer, persisted to `localStorage`. Web-first.                                                                                                                                                                                                                                                   | Active slice implemented: theme mode state in web app (`light`/`dark`) with system-default initialization, `localStorage` persistence, and manual sidebar toggle; core `ui-styles.ts` surfaces switched to theme CSS variables with light/dark palettes. Automated checks PASS: `pnpm --filter @widados/web lint`, `pnpm --filter @widados/web typecheck`. Remaining QA: full route visual pass in both modes. |
