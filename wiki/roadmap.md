...
| D1  | P2       | IN_PROGRESS | external    | B8         | Create Netlify site for web app.                                                                                                | `wiki/operations.md` D1: login + `pnpm netlify:sites:create -- -n <name> --json` (or `pnpm netlify:link`). Evidence when done: **Netlify site URL** (from JSON or dashboard).                                                                                  |
...
- 2026-04-24: **D1** in progress (CLI): `pnpm netlify:sites:create` added; `wiki/operations.md` documents login + `sites:create --json` vs `netlify:link`. Blocked on Netlify CLI auth until `netlify login` or token completes on this machine.
