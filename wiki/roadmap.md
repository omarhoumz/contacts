...
| D3  | P2       | IN_PROGRESS | external+code    | D2,B5      | Add Supabase env vars in Netlify.                                                                                               | `pnpm netlify:env:check` / `pnpm netlify:env:push` / `pnpm netlify:env:list`. **Blocked until** `apps/backend/supabase/.env.cloud` exists with hosted `SUPABASE_*`; then push + **`pnpm netlify:env:check` exit 0** + prod deploy.                                                                                |
...
- 2026-04-25: **D3** tooling: `pnpm netlify:env:push` / `pnpm netlify:env:list` (avoid root monorepo picker); `wiki/operations.md` + `wiki/deployment.md` updated. Awaiting filled `.env.cloud` + push + redeploy to close D3.
