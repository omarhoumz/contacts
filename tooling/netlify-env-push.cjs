...
/**
 * Push Supabase URL + publishable key from apps/backend/supabase/.env.cloud to Netlify
 ...
 *
 * Prereq: copy .env.cloud.example → .env.cloud and fill SUPABASE_URL + SUPABASE_PUBLISHABLE_KEY.
...
if (!fs.existsSync(cloudEnvPath)) {
  ...
  console.error("Copy apps/backend/supabase/.env.cloud.example to .env.cloud and set SUPABASE_URL + SUPABASE_PUBLISHABLE_KEY.");
  process.exit(1);
}
...
netlifyEnvSet("VITE_SUPABASE_PUBLISHABLE_KEY", publishable, ["--secret"]);
